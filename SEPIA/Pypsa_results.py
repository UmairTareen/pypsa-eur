...
def plot_demands(countries):
    ...
    for country in countries:
        ...
        logo['y']=1.021
        if country == 'BE':
         pio.write_image(fig, "results/pdf/Demands.pdf", format='pdf')
...
def plot_map(
    ...
    regions = gpd.read_file(f"resources/{study}/regions_onshore_elec_s_6.geojson").set_index("name")
    ...
    fig.legend(
        ...
        bbox_to_anchor=(0.8, 0.25),    # Centered horizontally and placed below the figure
...
def plot_ch4_map(network):
    ...
    for country in countries:
     gas_val = pd.read_csv(f"results/{study}/country_csvs/total_imports_{country}.csv", index_col=0)
     gas_val = gas_val[["imp_gaz_pe"]]
     ...
     if len(gas_val) >= len(desired_years):
      ...
      if planning_horizon in gas_val.index:
            gas_data[country] = gas_val.loc[planning_horizon, "imp_gaz_pe"]
...
def create_combined_chart_country(costs,investment_costs, capacities, s_capacities, country):
    ...
    heat_dispatch_sum_desc = country_desc.get(f"{country}_heat_dispatch_sum_desc", '')
    power_dispatch_win_desc = country_desc.get(f"{country}_power_dispatch_win_desc", '')
    power_dispatch_sum_desc = country_desc.get(f"{country}_power_dispatch_sum_desc", '')
    map_plots_desc = country_desc.get(f"{country}_map_plots_desc", '')
    h2_map_plots_desc = country_desc.get(f"{country}_h2_map_plots_desc", '')
    gas_map_plots_desc = country_desc.get(f"{country}_gas_map_plots_desc", '')
