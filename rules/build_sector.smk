...
rule build_energy_totals:
    ...
    output:
        ...
        district_heat_share=resources("district_heat_share_s{simpl}_{clusters}_{planning_horizons}.csv"),
    threads: 16
    resources:
        mem_mb=10000,
    log:
...
rule build_heat_totals:
    ...
    output:
        heat_totals=resources("heat_totals_s{simpl}_{clusters}_{planning_horizons}.csv"),
    threads: 1
    resources:
        mem_mb=2000,
    log:
        logs("build_heat_totals_s{simpl}_{clusters}_{planning_horizons}.log"),
    benchmark:
        benchmarks("build_heat_totals_s{simpl}_{clusters}_{planning_horizons}")
    conda:
        "../envs/environment.yaml"
    script:
...
rule build_population_weighted_energy_totals:
    params:
        ...
        snapshots=config_provider("snapshots"),
    input:
        energy_totals=resources("{kind}_totals_s{simpl}_{clusters}_{planning_horizons}.csv"),
        clustered_pop_layout=resources("pop_layout_elec_s{simpl}_{clusters}.csv"),
    output:
        resources("pop_weighted_{kind}_totals_s{simpl}_{clusters}_{planning_horizons}.csv"),
    threads: 1
    resources:
        mem_mb=2000,
    log:
        logs("build_population_weighted_{kind}_totals_s{simpl}_{clusters}_{planning_horizons}.log"),
    conda:
        "../envs/environment.yaml"
    script:
