...
OUTPUT = "/home/umair/Results/sankeys/Neww0M/"
...
n = pypsa.Network(f"{PATH}results/postnetworks/{SCENARIO}.nc")

fn = f"{PATH}resources/industrial_energy_demand_elec_s_6_2050.csv"
feedstock_emissions = (
...
# Select the two rows you want to sum based on conditions
row1 = gen[(gen["source"] == "offwind-ac") & (gen["target"] == "AC") & (gen["label"] == "offwind-ac")]["value"]
row2 = gen[(gen["source"] == "offwind-dc") & (gen["target"] == "AC") & (gen["label"] == "offwind-dc")]["value"]



# Check if the selected rows exist
matching_rows = gen[
    (gen["source"] == "offwind-ac")
    & (gen["target"] == "AC")
    & (gen["label"] == "offwind-ac")
]

if not matching_rows.empty:
    # Rows satisfying the conditions exist
    row1 = matching_rows.iloc[0]["value"]

    # Repeat the process for the second row
    matching_rows = gen[
        (gen["source"] == "offwind-dc")
        & (gen["target"] == "AC")
        & (gen["label"] == "offwind-dc")
    ]

    if not matching_rows.empty:
        row2 = matching_rows.iloc[0]["value"]

        # Calculate the sum of the values
        sum_value = row1 + row2

        # Create a new row with the summed value
        ...
        gen = pd.concat([gen, new_row], ignore_index=True)
    else:
        print("Second row not found.")
else:
    print("First row not found.")
    
gen = gen.drop(gen[(gen["label"] == "offwind-ac") | (gen["label"] == "offwind-dc")].index)

# Reset the index after dropping rows
gen = gen.reset_index(drop=True)

# Check if the new row exists in the DataFrame
new_row_condition = (gen["label"] == "Offshore Wind") & (gen["source"] == "Offshore Wind") & (gen["target"] == "AC")
if gen[new_row_condition].empty:
    print("New row not found.")
else:
    print("New row exists.")
    
####
row3 = gen[(gen["source"] == "solar") & (gen["target"] == "AC") & (gen["label"] == "solar")]["value"]
row4 = gen[(gen["source"] == "solar rooftop ") & (gen["target"] == "AC") & (gen["label"] == "solar rooftop")]["value"]
matching_rows = gen[
    (gen["source"] == "solar")
    & (gen["target"] == "AC")
    & (gen["label"] == "solar")
...
if not matching_rows.empty:
    # Rows satisfying the conditions exist
    row3 = matching_rows.iloc[0]["value"]

    # Repeat the process for the second row
    matching_rows = gen[
        (gen["source"] == "solar rooftop")
        & (gen["target"] == "low voltage")
        & (gen["label"] == "solar rooftop")
    ]

    if not matching_rows.empty:
        row4 = matching_rows.iloc[0]["value"]
...
gen = gen.drop(gen[(gen["label"] == "solar rooftop") | (gen["label"] == "solar")].index)

# Reset the index after dropping rows
gen = gen.reset_index(drop=True)

# Check if the new row exists in the DataFrame
new_row_condition = (gen["label"] == "Solar Power") & (gen["source"] == "Solar") & (gen["target"] == "AC")
if gen[new_row_condition].empty:
    print("New row not found.")
else:
    print("New row exists.")
...
###
row5 = gen[(gen["source"] == "solar") & (gen["target"] == "AC") & (gen["label"] == "solar")]["value"]
row6 = gen[(gen["source"] == "solar rooftop ") & (gen["target"] == "AC") & (gen["label"] == "solar rooftop")]["value"]
matching_rows = gen[
    (gen["source"] == "solar")
    & (gen["target"] == "AC")
    & (gen["label"] == "solar")
...
row5 = gen[(gen["source"] == "ror") & (gen["target"] == "AC") & (gen["label"] == "ror")]["value"]
row6 = su[(su["source"] == "hydro") & (su["target"] == "AC") & (su["label"] == "hydro")]["value"]
matching_rows = gen[
    (gen["source"] == "ror")
    & (gen["target"] == "AC")
    & (gen["label"] == "ror")
...
if not matching_rows.empty:
    # Rows satisfying the conditions exist
    row5 = matching_rows.iloc[0]["value"]
    ...
    if not matching_rows.empty:
        row6 = matching_rows.iloc[0]["value"]
...
gen = gen.drop(gen[(gen["label"] == "ror")].index)
                
# Reset the index after dropping rows
gen = gen.reset_index(drop=True)

# Check if the new row exists in the DataFrame
new_row_condition = (gen["label"] == "Hydro") & (gen["source"] == "Hydro") & (gen["target"] == "AC")
if gen[new_row_condition].empty:
    print("New row not found.")
else:
    print("New row exists.")
...
load.loc[load.label.str.contains("industry electricity") & (load.label == "industry electricity"), "target"] = "Industry"
load.loc[load.label.str.contains("H2 for industry") & (load.label == "H2 for industry"), "target"] = "Industry"
load.loc[load.label.str.contains("naphtha for industry") & (load.label == "naphtha for industry"), "target"] = "Non-energy"
load.loc[load.label.str.contains("land transport fuel cell") & (load.label == "land transport fuel cell"), "target"] = "Domestic transport"
load.loc[load.label.str.contains("land transport EV") & (load.label == "land transport EV"), "target"] = "Domestic transport"
load.loc[load.label.str.contains("agriculture electricity") & (load.label == "agriculture electricity"), "target"] = "Agriculture"
load.loc[load.label.str.contains("agriculture heat") & (load.label == "agriculture heat"), "target"] = "Agriculture"
load.loc[load.label.str.contains("urban central heat") & (load.label == "urban central heat"), "target"] = "Residential and Tertiary"
load.loc[load.label.str.contains("urban central heat") & (load.label == "urban central heat"), "source"] = "District Heating"
load.loc[load.label.str.contains("low-temperature heat for industry") & (load.label == "low-temperature heat for industry"), "target"] = "Industry"
load.loc[load.label.str.contains("low-temperature heat for industry") & (load.label == "low-temperature heat for industry"), "source"] = "District Heating"
...
df.loc[df.label.str.contains("urban central gas CHP") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("urban central gas CHP CC") & (df.source == "gas"), "source"] = "gas"
df.loc[df.label.str.contains("urban central gas CHP CC") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("urban central air heat pump") & (df.source == "Ambient Heat"), "source"] = "Ambient Heat"
df.loc[df.label.str.contains("urban central air heat pump") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("urban central gas boiler") & (df.source == "gas"), "source"] = "gas"
df.loc[df.label.str.contains("urban central gas boiler") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("urban central resistive heater") & (df.source == "low voltage"), "source"] = "low voltage"
df.loc[df.label.str.contains("urban central resistive heater") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("urban central water tanks discharger") & (df.source == "urban central water tanks"), "source"] = "urban central water tanks"
df.loc[df.label.str.contains("urban central water tanks discharger") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("urban central water tanks charger") & (df.source == "District Heating"), "source"] = "District Heating"
df.loc[df.label.str.contains("urban central water tanks charger") & (df.target == "urban central water tanks"), "source"] = "District Heating"
...
df.loc[df.label.str.contains("urban central solid biomass CHP") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("urban central solid biomass CHP CC") & (df.source == "biomass"), "source"] = "biomass"
df.loc[df.label.str.contains("urban central solid biomass CHP CC") & (df.target == "urban central heat"), "target"] = "District Heating"
df.loc[df.label.str.contains("residential rural biomass boiler") & (df.source == "solid biomass"), "source"] = "biomass"
df.loc[df.label.str.contains("residential rural biomass boiler") & (df.target == "urban central heat"), "target"] = "heat"
df.loc[df.label.str.contains("services rural biomass boiler") & (df.source == "biomass"), "source"] = "biomass"
df.loc[df.label.str.contains("services rural biomass boiler") & (df.target == "urban central heat"), "target"] = "heat"
df.loc[df.label.str.contains("residential urban decentral biomass boiler") & (df.source == "biomass"), "source"] = "biomass"
df.loc[df.label.str.contains("residential urban decentral biomass boiler") & (df.target == "urban central heat"), "target"] = "heat"
df.loc[df.label.str.contains("services urban decentral biomass boiler") & (df.source == "biomass"), "source"] = "biomass"
df.loc[df.label.str.contains("services urban decentral biomass boiler") & (df.target == "urban central heat"), "target"] = "heat"
...
df.loc[df.label.str.contains("solid biomass for industry") & (df.target == "solid biomass for industry"), "target"] = "Industry"
df.loc[df.label.str.contains("solid biomass for industry CC") & (df.source == "biomass"), "source"] = "biomass"
df.loc[df.label.str.contains("solid biomass for industry CC") & (df.target == "solid biomass for industry CC"), "target"] = "Industry"
...
df.loc[df.label.str.contains("urban central water tanks charger") & (df.source == "urban central heat"), "source"] = "District Heating"
df.loc[df.label.str.contains("urban central water tanks discharger") & (df.target == "urban central water tanks"), "source"] = "TES Central"
...
where = connections.label == "urban central solid biomass CHP"
...
collection.append(
    pd.Series(
        dict(
            label="solid biomass biomass to liquid", source="biomass", target="oil", value=value  #C02 stored in oil by BTL from costs data 2050
        )
    )
)
...
collection.append(
    pd.Series(
        dict(
            label="solid biomass biomass to liquid", source="biomass", target="co2 atmosphere", value=value * 0.26 #C02 stored in oil by BTL from costs data 2050
        )
    )
)
...
value = -(
    n.snapshot_weightings.generators @ n.links_t.p2.filter(like="solid biomass biomass to liquid CC")
).sum()
collection.append(
    pd.Series(
        dict(
            label="solid biomass biomass to liquid CC",
            source="oil",
            target="co2 atmosphere",
            value=value,
        )
...
collection.append(
    pd.Series(
        dict(
            label="solid biomass biomass to liquid CC", source="co2 atmosphere", target="oil", value=value
        )
    )
)
...
collection.append(
    pd.Series(
        dict(
            label="solid biomass solid biomass to gas", source="biomass", target="gas", value=value# CO2 stored in bioSNG from cost data
        )
    )
)
collection.append(
...
collection.append(
    pd.Series(
        dict(
            label="solid biomass solid biomass to gas", source="biomass", target="co2 atmosphere", value=value * 0.23# CO2 stored in bioSNG from cost data
        )
    )
)
collection.append(
...
collection.append(
    pd.Series(
        dict(
            label="gas for industry", source="gas", target="co2 atmosphere", value=value * 0.2  # C02 intensity of gas from cost data
        )
    )
)
...
collection.append(
    pd.Series(
        dict(label=tech, source="biomass", target="co2 atmosphere", value=value * 0.37)
    )
)
collection.append(
...
# collection.append(
#     pd.Series(dict(label="net co2 emissions", source="net co2 emissions", target="solid biomass", value=value))
# )
...
value = (n.snapshot_weightings.generators @ n.links_t.p0.filter(like="biomass")).div(
    1e6
).sum() * 0.37
row = pd.DataFrame(
    [dict(label="biomass", source="solid biomass", target="biomass", value=value)]
)
df = pd.concat([df, row], axis=0)



colors["net co2 emissions"] = "greenyellow"
