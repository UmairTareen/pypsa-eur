...
def prepare_sankey(n):
    ...
    load.loc[
    load.label.str.contains("land transport oil") & (load.label == "land transport oil"), "target"] = "Domestic transport"
    ...
    connections.replace("shipping oil", "Maritime bunkers", inplace=True)
...
def plot_sankey(connections):
    ...
    colors["solid biomass powerplants"] = colors["biomass"]
...
def prepare_carbon_sankey(n):
    ...
    value = (n.snapshot_weightings.generators @ n.generators_t.p.filter(like="biomass")).sum() * 0.37
    collection.append(
        pd.Series(dict(label="solid biomass powerplants", source="solid biomass", target="co2 atmosphere", value=value))
    )
    collection.append(
        pd.Series(dict(label="solid biomass powerplants", source="co2 atmosphere", target="solid biomass", value=value))
    )
    # Sabatier
...
if __name__ == "__main__":
    if "snakemake" not in globals():
        ...
        snakemake = mock_snakemake(
            ...
            clusters="6",
            ll="v1.0",
            sector_opts="Co2L0.8-1H-T-H-B-I-A-dist1",
