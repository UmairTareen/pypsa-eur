...
def define_spatial(nodes, options):
    ...
    if options["co2_spatial"]:
        ...
        spatial.co2.LULUCF = nodes + " LULUCF"
    else:
...
def add_co2_tracking(n, options):
    ...
    n.madd(
        "Store",
        spatial.co2.nodes,
        e_nom_extendable=False,                                    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>CO2 storage
        e_nom_max=e_nom_max,
...
def insert_electricity_distribution_grid(n, costs):
    ...
    potential = 0.25 * 10 * pop_solar                          #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>solar rooftop
    ...
    if options.get("coal_cc"):
        n.madd(
            ...
            * costs.at["coal", "CO2 intensity"],  # NB: fixed cost is per MWel
            p_nom_extendable=False,                                                #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>coal CC
            carrier="coal",
    ...
    for name in heat_systems:
        ...
        if options["chp"] and name == "urban central":
            ...
            n.madd(
                ...
                p_nom_extendable=False,                                    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>gas CHP CC
                capital_cost=costs.at["central gas CHP", "fixed"]
                * costs.at["central gas CHP", "efficiency"]
                + costs.at["biomass CHP capture", "fixed"]
                * costs.at["gas", "CO2 intensity"],
...
def add_biomass(n, costs):
    ...
    if not urban_central.empty and options["chp"]:
        ...
        n.madd(
            ...
            p_nom_extendable=False,                                                #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>biomass CHP CC
            capital_cost=costs.at[key, "fixed"] * costs.at[key, "efficiency"]
            + costs.at["biomass CHP capture", "fixed"]
    ...
    if options["biomass_to_liquid"]:
        ...
        n.madd(
            ...
            p_nom_extendable=False,                                                             #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>biomass to liquid CC
            capital_cost=costs.at["BtL", "fixed"]
    ...
    if options["biosng"]:
        ...
        n.madd(
            ...
            * (1 - costs.at["BioSNG", "capture rate"]),
            p_nom_extendable=False,                                      #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>biomass to gas CC
            capital_cost=costs.at["BioSNG", "fixed"]
            + costs.at["biomass CHP capture", "fixed"]
            * costs.at["BioSNG", "CO2 stored"],
            marginal_cost=costs.at["BioSNG", "efficiency"] * costs.loc["BioSNG", "VOM"],
...
def add_industry(n, costs):
    ...
    n.madd(
        ...
        p_nom_extendable=False,                                      #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>biomass for industry CC
    ...
    n.madd(
        ...
        p_nom_extendable=False,                             #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>gas for industry CC
        capital_cost=costs.at["cement capture", "fixed"]
        * costs.at["gas", "CO2 intensity"],
        efficiency=0.9,
        efficiency2=costs.at["gas", "CO2 intensity"]
    ...
    n.madd(
        ...
        p_nom_extendable=True,                                            #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>process emissions CC
        capital_cost=costs.at["cement capture", "fixed"],
